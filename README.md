...
### 🆕 Киллер-фичи (v1.8.9, взяты из самых популярных AI-компаньонов)

- **💜 Уровни отношений и XP** (как в Replika): за каждое сообщение Лилит
  начисляет XP — 7 уровней: Знакомство → Дружба → Лёгкий флирт → Романтика →
  Страсть → Любовь → Родные души. Прогресс виден в `/profile` и в Mini App.
- **🔄 Свайпы «Другой ответ»** (как в Character.AI / SillyTavern): под каждым
  ответом Лилит в Telegram — кнопка, а в Mini App — кнопка 🔄. Один тап —
  и она отвечает иначе.
- **📖 Дневник Лилит** (как в Replika): каждый вечер Лилит пишет запись
  в личный дневник по мотивам вашего дня. Читается в Mini App (вкладка 📖)
  и по команде `/diary`.
- **📚 Книга мира (лорбук)** (как в SillyTavern / HammerAI): у Лилит теперь
  есть стабильный «мир» — дом, кот Барон, любимая музыка, привычки.
  Записи подмешиваются в разговор, когда вы их касаетесь (см.
  `prompts/lorebook.md`).
- **🎛 Креативность и длина ответов** (как в open-character-ai): в настройках
  Mini App можно выбрать, насколько дерзко и подробно Лилит отвечает.
...
### 🆕 Аватары «в белье» и аниме (v1.8.12)

- **Реалистичный стиль в белье** — 19 эмоций из 23 (`assets/emotions/lingerie/`);
- **Аниме-стиль в белье** — 10 сексуальных эмоций (`lilith_{emotion}_anime_lingerie.png`):
  neutral, passion, flirt, playful, shy, happy, excited, tender, sad, angry;
- Для отсутствующих эмоций сервер подставляет **ближайшую по смыслу в белье**
  (карты `_ANIME_LINGERIE_FALLBACK` / `_LINGERIE_FALLBACK`), поэтому Лилит
  в режиме «в белье» ВСЕГДА в белье, а не одетая;
- **Все аватары Лилит сексуальные/соблазнительные** — кружево, чулки,
  подвязки, игривые позы (правило пользователя, закреплено в AI_DEVELOPER.md).

### 🆕 Mini App (v1.8.11)

- **Чат пузырями** — история диалога подгружается с сервера, сообщения как в мессенджере;
- **Быстрые подсказки** — готовые фразы над полем ввода («Покажи себя», «В белье»…);
- **Индикатор «Лилит печатает…»** при ожидании ответа;
- **Прогресс-бар уровня отношений** под аватаром;
- **«🩲 В белье»** — переключение наряда с умным подбором ближайшей эмоции, если файла ещё нет;
- Эмоции в белье: 19 из 23 (остальные подставляются ближайшими);
- Кнопка «🔄 Другой ответ» — свайпы вариантов.
