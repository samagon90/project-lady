...
1. Возьмите папку с проектом (архив `project-lady.zip` или папку, которую вам передали).
2. Если это архив — распакуйте его: кликните правой кнопкой → «Извлечь всё».
3. Переименуйте папку в короткое имя без пробелов: **`project-lady`**.
4. Положите её **прямо на рабочий стол** — так её будет легко найти.
5. Убедитесь, что внутри папки вы видите файл **`start_windows.bat`**
   (для Windows) или **`start.sh`** (для Mac).
...
В папке проекта есть файл-заготовка настроек. Нужно сделать его копию и вписать токен.

### Windows

1. Откройте папку `project-lady` в Проводнике.
2. Включите показ расширений файлов, иначе не увидите файл:
   вверху окна вкладка **«Вид»** → поставьте галочку **«Расширения имён файлов»**.
3. Найдите файл **`.env.example`**. Нажмите на него правой кнопкой →
   **«Переименовать»** → удалите слово `example` (вместе с точкой), чтобы
   получилось **`.env`** → нажмите Enter. (Окно «смените расширение?» — нажмите **Да**.)
4. Нажмите на **`.env`** правой кнопкой → **«Открыть с помощью»** → **«Блокнот»**.
5. Найдите строку `TELEGRAM_TOKEN=` и впишите сразу после `=` ваш токен
   от BotFather, **без пробелов**. Должно получиться как-то так:
   ```
   TELEGRAM_TOKEN=7123456789:AAHfK3x8yQWERTY...
   ```
6. Нажмите **Ctrl+S** (сохранить) и закройте Блокнот.

> Если файл не виден даже после шага 2 — просто запустите `start_windows.bat`,
> он сам создаст `.env`, и вы заполните его так же.
...
3. Откроется чёрное окно. Первый запуск занимает 2–5 минут
   (ставит вспомогательные программы) — **не закрывайте окно**.
4. Если бот попросит заполнить `.env` — сделайте как в шаге 4 и запустите файл ещё раз.
5. В конце вы увидите строку **«Бот запущен»**. Всё, он работает!
...
| «TELEGRAM_TOKEN пуст» | Токен не вписан | Шаг 4: вписать токен в `.env` и сохранить |
