...
async def test_miniapp_gallery_teacher(ctx) -> None:
    """Серия «Строгая учительница»: фото в галерее + отдача файла."""
    import hashlib
    import hmac
    import json as _json
    import urllib.parse
    ...
    token = "123:TESTTOKEN"
    await onboard(ctx, 7781, nsfw=True)
    server = MiniAppServer(ctx.db, token)

    user_json = _json.dumps({"id": 7781, "first_name": "T", "is_bot": False})
    pairs = [("user", user_json), ("auth_date", "1700000000")]
    data_check = "\n".join(f"{k}={v}" for k, v in sorted(pairs))
    secret = hmac.new(b"WebAppData", token.encode(), hashlib.sha256).digest()
    digest = hmac.new(secret, data_check.encode(), hashlib.sha256).hexdigest()
    ...
    assert len(teacher) >= 19, f"нужно 19 фото учительницы, найдено {len(teacher)}"

    resp2 = await server._api_gallery_static_image(_Req("lilith_teacher_01_blackboard.png"))
    assert resp2.status == 200


async def test_miniapp_gallery_boss(ctx) -> None:
    """Серия «Доминанта» (юбка-карандаш, ракурс снизу): 10 фото + отдача файла."""
    import hashlib
    import hmac
    import json as _json
    import urllib.parse
    ...
    token = "123:TESTTOKEN"
    await onboard(ctx, 7782, nsfw=True)
    server = MiniAppServer(ctx.db, token)

    user_json = _json.dumps({"id": 7782, "first_name": "T", "is_bot": False})
    pairs = [("user", user_json), ("auth_date", "1700000000")]
    data_check = "\n".join(f"{k}={v}" for k, v in sorted(pairs))
    secret = hmac.new(b"WebAppData", token.encode(), hashlib.sha256).digest()
    digest = hmac.new(secret, data_check.encode(), hashlib.sha256).hexdigest()
    ...
    resp = await server._api_gallery_static(_Req())
    data = _json.loads(resp.body)
    boss = [n for n in data["images"] if n.startswith("lilith_boss_")]
    assert len(boss) >= 10, f"нужно 10 фото доминанты, найдено {len(boss)}"
...
async def test_miniapp_gallery_anime_hot(ctx) -> None:
    """Серия «Горячее аниме»: фото в галерее + отдача файла."""
    import hashlib
    import hmac
    import json as _json
    import urllib.parse
    ...
    token = "123:TESTTOKEN"
    await onboard(ctx, 7784, nsfw=True)
    server = MiniAppServer(ctx.db, token)

    user_json = _json.dumps({"id": 7784, "first_name": "T", "is_bot": False})
    pairs = [("user", user_json), ("auth_date", "1700000000")]
    data_check = "\n".join(f"{k}={v}" for k, v in sorted(pairs))
    secret = hmac.new(b"WebAppData", token.encode(), hashlib.sha256).digest()
    digest = hmac.new(secret, data_check.encode(), hashlib.sha256).hexdigest()
    ...
    resp = await server._api_gallery_static(_Req())
    data = _json.loads(resp.body)
    anime_hot = [n for n in data["images"] if n.startswith("lilith_animehot_")]
    assert len(anime_hot) >= 7, f"нужно 7+ горячих аниме-фото, найдено {len(anime_hot)}"

    resp2 = await server._api_gallery_static_image(_Req("lilith_animehot_01_bed_recline.png"))
    assert resp2.status == 200
