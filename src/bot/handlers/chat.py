...
async def on_text(message: Message, bot: Bot, app_ctx: AppContext, user: DbUser | None) -> None:
    ...
    # Поздравление с «круглой» датой стрика (серия дней подряд)
    if result.streak_text:
        try:
            await bot.send_message(chat_id=message.chat.id, text=result.streak_text)
        except Exception:  # noqa: BLE001
            logger.warning("Не удалось отправить streak_text: %s", result.streak_text)

    # Новые достижения (геймификация)
    if result.achievements:
        try:
            ...
            logger.warning("Не удалось отправить achievements: %s", result.achievements)
...
def _detect_reply_emotion(text: str) -> str:
    ...
    v2.0: добавлены эмодзи, интонации («!»/«?»), обращения и разговорные
    маркеры, чтобы фото менялось почти при КАЖДОМ ответе, а не только
    когда в тексте есть слова-маркеры.
    """
    t = text.lower()
    # Эмодзи — самый надёжный маркер
    emoji_map = [
        ("😈|🔥|💋|👅|🫦|😏", "passion"),
        ("💗|🥰|💖|😍|❤️|🖤", "tender"),
        ("😭|🥺|💔|😢", "crying"),
    ...
    for pattern, emotion in emoji_map:
        if re.search(pattern, t):
            return emotion
    pairs = [
        (r"облегч|фух|слава богу|выдох|наконец-то спокойн", "relief"),
        (r"отврат|гадость|противн|мерзост|фу[ ,.!]|фу$|фу-фу", "disgust"),
        (r"презр|высокомер|снисход|фырк|свысока|пф[ ,.!]|пф$", "contempt"),
        (r"дума|размышл|интересн|хм|подумать|представь|вопрос", "thinking"),
        (r"не понял|не понимаю|запута|странн|объясни|что происходит", "confused"),
        (r"плач|груст|печал|обид|тоск|одинок|жаль|прости|слез", "crying"),
        (r"боюсь|страш|испуг|жутк|кошмар|опасн|жутко", "scared"),
        (r"зл|бес(ишь|ит|ить|у|ят)|ненавиж|разозл|ярост|недовольн|наказан", "angry"),
        (r"ревн|измен|другая|другой|кто эта", "jealous"),
        (r"горд|восхищ|молодец|круто|супер|топ|горжусь", "proud"),
        (r"скуч|устал|нудно|надоел|зев|скучно", "bored"),
        (r"сон|спат|спать|зев|засыпа", "sleepy"),
        (r"восторг|вау|обалдет|невероят|офигеть|класс|потрясн|какой восторг", "excited"),
        (r"смущ|стесн|красне|неловк|щекотн", "shy"),
        (r"удив|вот это да|ничего себе|неожидан|чтоо|правда\?|серьёзно\?|неужели", "surprised"),
        (r"рад|счаст|улыб|отлично|прекрасн|клёво|здорово|замечательн", "happy"),
        (r"хочу|страст|поцелуй|разде|гол|секс|эрот|ночь|жела|возбужд|госпож|покорн|слуг", "passion"),
        (r"нежн|мил|ласков|тёпл|тепл|скуча|обним|родн|мой хороший|котик|зайк", "tender"),
        (r"флирт|кокет|соблазн|красив|обольст|нрав|симпат|дразн", "flirt"),
        (r"шут|смеш|ха-ха|прикол|весел|хихи|ахах|лол|кек", "playful"),
        (r"серьез|серьёз|строг|важн|дело|слушай внимательно", "serious"),
        (r"люблю тебя|люблю,", "tender"),
    ]
    for pattern, emotion in pairs:
        if re.search(pattern, t):
            return emotion
    # Интонации: несколько «!» — восторг/злость, «?» — размышление
    if re.search(r"!{2,}", t):
        return "excited"
    if re.search(r"\?{1,}", t) and len(t) < 200:
        return "thinking"
    return "neutral"
