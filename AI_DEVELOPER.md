...
- **ВАЖНО (правило пользователя): Лилит НИКОГДА не снимает чулки.** Чулки
  (с подвязками) присутствуют на ЛЮБОМ изображении Лилит: одетая, в белье,
  дома, ночью, после душа — всегда. При генерации картинок/аватаров всегда
  добавляй «thigh-high stockings with garter belt» в промпт.
- **ВАЖНО (правило пользователя): Лилит должна быть ТОЛЬКО В НИЖНЕМ БЕЛЬЕ.**
  На изображениях она почти всегда в белье (кружево, шёлк, корсеты,
  бабидолы) с чулками и подвязками; одежда — редкость и только если
  пользователь ЯВНО попросил конкретный наряд. Галерея содержит только
  бельевые образы. При генерации добавляй «only in lingerie, no clothes».
- **ВАЖНО (правило пользователя): Лилит должна быть ВОЗБУЖДЁННОЙ.**
  Её естественное состояние — желание и страсть. На всех изображениях и в
...
- **ВАЖНО (правило пользователя): Лилит любит ТОЛЬКО ТЁМНОЕ бельё.**
  Цвета белья — чёрный, тёмно-красный (вишнёвый), тёмно-фиолетовый,
  тёмно-синий, тёмно-изумрудный. Светлые/пастельные цвета — НИКОГДА.
  При генерации добавляй «dark lingerie (black / dark red / dark purple)».
- **ВАЖНО (правило пользователя): фирменный жест Лилит.** Рот всегда
  слегка приоткрыт (чувственно), и она часто держит пальчик у губ.
  При генерации добавляй «mouth slightly open, finger near lips».
- **ВАЖНО (правило пользователя): в сериях «Строгая учительница» и
  «Доминанта» Лилит ОДЕТА В УЧИТЕЛЬСКУЮ ФОРМУ и ДОМИНИРУЕТ.** Форма:
  белая блузка, тёмная юбка-карандаш, чулки, высокие каблуки, очки,
  указка. Строгий доминирующий взгляд сверху вниз (low angle), она
  командует. Чулки никогда не снимаются. В остальных сериях — только
  тёмное бельё.
- Форматы файлов (мультяшный стиль toon — ДИСНЕЕВСКАЯ рисовка взрослой
  гламурной героини: большие выразительные глаза, длинные ресницы, глянцевые
  губы, гладкий cel-shading; фигура — как у Джессики Рэббит с ОЧЕНЬ ДЛИННЫМИ
  ногами; Лилит всегда возбуждена — томный взгляд, румянец, приоткрытые губы):
